...
def drawGraph(G:nx.Graph,position:dict[str,Point],edgeLabels:dict[tuple[str,str],str],A: list[tuple[str, str]]) -> None:
    font_size = 24
    node_size = 5000
    edge_width = 5.
    node_color = "c"
    ...
    plt.subplot(2, 1, 1)
    nx.draw_networkx_nodes(G, position, node_size = node_size,
    ...
    nx.draw_networkx_nodes(G, position, node_size = node_size,
        node_color = node_color)
    nx.draw_networkx_labels(G, position, font_size = font_size)
    nx.draw_networkx_edges(G, position, A, width = edge_width,
        edge_color = 'r', arrows = True, arrowsize = arrowsize,
        node_size = node_size)
